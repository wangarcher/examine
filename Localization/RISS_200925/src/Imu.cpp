...
ImuData::ImuData()
{
	float x_,y_,theta_;
    //陀螺仪初始化
    this->Gyro.gravity_x = 0.0;
    this->Gyro.gravity_y = 0.0;
    this->Gyro.gravity_z = 0.0;

    //加速度计初始化
    this->Accelerometer.accelerate_x = 0.0;
    this->Accelerometer.accelerate_y = 0.0;
    this->Accelerometer.accelerate_z = 0.0;
	
	this->UpDateBenchmarkYPR.Roll = 0.0;
	this->UpDateBenchmarkYPR.Pitch = 0.0;
	this->UpDateBenchmarkYPR.Yaw = 0.0;

    this->LastAccelerometer.accelerate_x = 0.0;
    this->LastAccelerometer.accelerate_y = 0.0;
    this->LastAccelerometer.accelerate_z = 0.0;

    this->Position.x = 0.0;
    this->Position.y = 0.0;
    this->Position.theta = 0.0;

    this->PostureYPR.Pitch = 0.0;
    this->PostureYPR.Roll = 0.0;
    this->PostureYPR.Yaw = 0.0;

    this->Quaternionelement.q0 = 0.0;
    this->Quaternionelement.q1 = 0.0;
    this->Quaternionelement.q2 = 0.0;
    this->Quaternionelement.q3 = 0.0;
...
int ImuData::InitImu()
{
	...
	//******************UDP通信初始化**************************//
    Imufd = open("/dev/ttyUSB0",O_RDWR | O_NOCTTY | O_NONBLOCK);

    if(Imufd <= 0)
    {
        std::cout << "Fail to open /dev/ttyUSB0!" << std::endl;
        return -1;
    }
